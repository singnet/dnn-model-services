...
class VideoCaptioner:
    ...
    # Downloads the video from YouTube URL (using pytube).
    def _download_yt_video(self):
        try:
            video_path = YouTube(self.url).streams.filter(
                progressive=True,
    ...
    def get_captions(self):
        ...
        try:
            ...
            ok, video_path = self._download_yt_video()
            if ok:
                # Get all frames within interval start_time and stop_time.
                ok, frames_list = get_video_frames(video_path,
                                                   self.video_folder,
                ...
                if ok:
                    ...
                    if self.batch_size == 0:
                        ...
                        if self.batch_size > 60:
                            ...
                            return {'Error': 'batch_size is too high! (max: 20s)'}
                    ...
                    result = {'Error': 'Fail at "get_video_frames()"'}
            else:
                result = {'Error': 'Fail at "_download_yt_video()"'}
