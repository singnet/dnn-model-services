...
from snet_control import SnetInstance
...
class DetectRecon(SnetInstance):
    ...
    def detect_recon(self):
        ...
        try:
            ...
            #
            ###########
            if self.snet_call_res_json:
                bboxes = self.snet_call_res_json["objDetect"]["response"]["boxes"]
                if bboxes == "Fail":
                    log.info("ObjectDetector fail!")
                else:
                    ...
                    for item in bboxes:
                        if not os.path.exists("temp"):
                            os.makedirs("temp")
                        log.info("Croping bbox {}...".format(bboxe_idx))
                        coords = (
                            item[0],
                            item[1],
                            item[0] + item[2],
                        ...
                        bbox_base64 = iutils.crop_image(self.img_path, coords, "base64")
