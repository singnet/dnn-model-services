...
import service.resnet_cntk_image_recognition as img_recon
...
from service.model.resnet_img_recon_pb2 import Result

logging.basicConfig(
    level=10, format="%(asctime)s - [%(levelname)8s] - %(name)s - %(message)s")
log = logging.getLogger('img_recon')
...
class FlowersServicer(grpc_bt_grpc.FlowersServicer):
    ...
    def flowers(self, request, context):
        ...
        self.result = Result()
        self.result.top_5 = json_result['top_5']
        self.result.delta_time = json_result['delta_time']
        log.debug('flowers({},{})={}'.format(
            self.model, self.img_path, self.result.top_5))
        return self.result
...
def serve(max_workers=10, port=7777):
    ...
    grpc_bt_grpc.add_FlowersServicer_to_server(
        FlowersServicer(), server)
    grpc_bt_grpc.add_DogsServicer_to_server(
        DogsServicer(), server)
    grpc_bt_grpc.add_CarsServicer_to_server(
        CarsServicer(), server)
